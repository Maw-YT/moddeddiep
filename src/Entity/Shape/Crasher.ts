...
export default class Crasher extends AbstractShape {
    ...
    public constructor(game: GameServer, large=false, isAlpha=false) {
        super(game);

        this.nameData.values.name = isAlpha ? "Alpha Crasher" : "Crasher";
        ...
        this.healthData.values.health = this.healthData.values.maxHealth = isAlpha ? this.healthData.values.health * 3 : this.healthData.values.health;
        this.physicsData.values.size = (large ? 55 : 35) * Math.SQRT1_2;
        this.physicsData.values.size = isAlpha ? this.physicsData.values.size * 1.5 : this.physicsData.values.size;
        this.physicsData.values.sides = 3;
        this.physicsData.values.absorbtionFactor = large ? 0.1 : 2;
        this.physicsData.values.absorbtionFactor = isAlpha ? this.physicsData.values.absorbtionFactor * 0.75 : this.physicsData.values.absorbtionFactor;
        this.physicsData.values.pushFactor =  large ? 12 : 8;
        this.physicsData.values.pushFactor = isAlpha ? this.physicsData.values.pushFactor * 1.5 : this.physicsData.values.pushFactor;

        this.styleData.values.color = Color.EnemyCrasher;

        this.scoreReward = large ? 25 : 15;
        this.scoreReward = isAlpha ? this.scoreReward * 3 : this.scoreReward;
        this.damagePerTick = isAlpha ? 4 : 2;
        this.isLarge = large;
        this.targettingSpeed = large ? 2.64 : 2.602;
        this.targettingSpeed = isAlpha ? this.targettingSpeed * 1.25 : this.targettingSpeed;
